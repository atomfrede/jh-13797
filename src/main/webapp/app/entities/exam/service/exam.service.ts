...
export class ExamService {
  ...
  update(exam: IExam): Observable<EntityResponseType> {
    return this.http.put<IExam>(`${this.resourceUrl}/${getExamIdentifier(exam) as string}`, exam, { observe: 'response' });
  }

  partialUpdate(exam: IExam): Observable<EntityResponseType> {
    return this.http.patch<IExam>(`${this.resourceUrl}/${getExamIdentifier(exam) as string}`, exam, { observe: 'response' });
  }
